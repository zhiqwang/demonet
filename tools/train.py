...
from models.utils import (
    init_seeds, plot_images, print_mutation,
    plot_results, create_backbone, print_model_biases,
    labels_to_image_weights, fitness, attempt_download,
    load_darknet_weights,
)
from models.losses import compute_loss
from datasets.yolov3 import LoadImagesAndLabels

mixed_precision = False  # not installed
if mixed_precision:  # Mixed precision training https://github.com/NVIDIA/apex
...
# Overwrite param with param*.txt (optional)
f = glob.glob('param*.txt')
if f:
    for k, v in zip(param.keys(), np.loadtxt(f[0])):
        param[k] = v
...
if __name__ == '__main__':
    ...
    parser.add_argument('--multi-scale', action='store_true', help=r'adjust (67% - 150%) img_size every 10 batches')
    ...
    parser.add_argument('--arch', type=str, default='default', help='yolo architecture')  # defaultpw, uCE, uBCE
    ...
    # scale param['obj'] by img_size (evolved at 416)
    param['obj'] *= opt.img_size / 416.
        ...
        for _ in range(1):  # generations to evolve
            if os.path.exists('evolve.txt'):  # if evolve.txt exists: select best hyps and mutate
                ...
                for i, k in enumerate(param.keys()):
                    ...
                    param[k] *= float(x)  # vary by sigmas

            # Clip to limits
            keys = ['lr0', 'iou_t', 'momentum', 'weight_decay', 'hsv_s', 'hsv_v', 'translate', 'scale', 'fl_gamma']
            limits = [(1e-5, 1e-2), (0.00, 0.70), (0.60, 0.98), (0, 0.001), (0, .9), (0, .9), (0, .9), (0, .9), (0, 3)]
            for k, v in zip(keys, limits):
                param[k] = np.clip(param[k], v[0], v[1])
            ...
            print_mutation(param, results, opt.bucket)

            # Plot results
            # plot_evolution_results(param)
