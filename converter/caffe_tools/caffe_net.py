...
class _Net(object):
    ...
    def layer_index(self, layer_name):
        # find a layer's index by name. if the layer was found
        # return the layer position in the net, else return -1.
...
class Prototxt(_Net):
    ...
    def init_caffemodel(self, caffe_cmd_path='caffe'):
        """
        :param caffe_cmd_path: The shell command of caffe
            normally at <path-to-caffe>/build/tools/caffe
        """
        ...
        with open('/tmp/nn_tools_solver.prototxt', 'w') as f:
            f.write(str(s))
        import os
        os.system('{} train --solver /tmp/nn_tools_solver.prototxt'.format(caffe_cmd_path))
