...
class LoadImagesAndLabels(data.Dataset):  # for training/testing
    def __init__(
        self, path, img_size=416, batch_size=16,
        augment=False, param=None, rect=True,
        image_weights=False, cache_labels=False,
        cache_images=False,
    ):
        path = str(Path(path))  # os-agnostic
        ...
        if cache_labels or image_weights:  # cache labels for faster training
            ...
            for i, file in enumerate(pbar):
                ...
                pbar.desc = 'Reading labels ({} found, {} missing, {} empty for {} images)'.format(nf, nm, ne, n)
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            # Augment colorspace
            augment_hsv(img, hgain=self.param['hsv_h'], sgain=self.param['hsv_s'], vgain=self.param['hsv_v'])

            # Augment imagespace
            g = 0.0 if mosaic else 1.0  # do not augment mosaics
            param = self.param
            img, labels = random_affine(img, labels,
                                        degrees=param['degrees'] * g,
                                        translate=param['translate'] * g,
                                        scale=param['scale'] * g,
                                        shear=param['shear'] * g)
