...
from demonet.data import xywh2xyxy
from demonet.utils import torch_utils  # , google_utils
...
def build_targets(model, targets):
    ...
    for i in model.yolo_layers:
        ...
        if nt:
            ...
            reject = True
            if reject:
                j = iou > model.param['iou_t']  # iou threshold hyperparameter
...
def kmeans_targets(path='../coco/trainvalno5k.txt', n=9, img_size=512):  # from utils.utils import *; kmeans_targets()
    ...
    from demonet.data.coco import LoadImagesAndLabels
...
def print_mutation(param, results, bucket=''):
    # Print mutation results to evolve.txt (for use with train.py --evolve)
    a = '%10s' * len(param) % tuple(param.keys())  # hyperparam keys
    b = '%10.3g' * len(param) % tuple(param.values())  # hyperparam values
...
def plot_evolution_results(param):  # from utils.utils import *; plot_evolution_results(param)
