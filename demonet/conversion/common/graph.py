...
class Graph(object):
    ...
    def get_parent_variable_name(self, name, path, set_flag=False):
        ...
        for idx in path:
            if len(current_node.in_edges) <= idx:
                return None
            parent_name = current_node.in_edges[idx].split(':')[0]
            if len(current_node.in_edges[idx].split(':')) == 1:
                current_subscriptor = ''
            else:
                current_subscriptor = '[{}]'.format(current_node.in_edges[idx].split(':')[1])
            current_node = self.get_node(parent_name)
            if set_flag:
                current_node.covered = True
    ...
    def _get_topological_sort(self):
        ...
        while idx < len(self.topological_sort):
            ...
            for next_node in current_node.out_edges:
                ...
                next_node_info.left_in_edges -= self._check_left_in_edges_num(
                    current_node.name, next_node_info,
                )
    ...
    def _make_connection(self, src, dst):
        ...
        if dst not in self.layer_map[src.split(':')[0]].out_edges:
