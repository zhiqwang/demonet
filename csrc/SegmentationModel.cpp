#include <csrc/resnet.h>
#include <csrc/SegmentationModel.h>
#include <csrc/DeeplabV3.h>
...
void SegmentationModelImpl::fcn_resnet50(bool pretrained, int64_t num_classes, bool aux) {
  ...
  _backbone = IntermediateLayerGetter(
      IntermediateLayerGetterImpl(std::move(Resnet), { "layer3","layer4" }));
...
void SegmentationModelImpl::fcn_resnet101(
    bool pretrained,
    int64_t num_classes,
    bool aux) {
  _classifier = _make_FCNHead(2048, num_classes);
...
void SegmentationModelImpl::deeplabv3_resnet101(
    bool pretrained,
    int64_t num_classes,
    bool aux) {
  ...
  _classifier = torch::nn::Sequential(
      ...
      torch::nn::BatchNorm2d(torch::nn::BatchNormOptions(256).eps(0.001).momentum(0.01)),
...
void SegmentationModelImpl::deeplabv3_resnet50(
    bool pretrained,
    int64_t num_classes,
    bool aux) {
  int64_t in_channels = 2048;
...
torch::nn::Sequential SegmentationModelImpl::_make_FCNHead(
    int64_t in_channels,
    int64_t channels) {
  ...
  return torch::nn::Sequential(
      ...
      torch::nn::Conv2d(torch::nn::Conv2dOptions(inter_channels_, channels, 1)));
}
